...
Public Class frmSaleReturn
    ...
    Private Sub SaveDetails()
        ...
        If lblID.Text <> "" Then
            cnn.executeSQL("DELETE From SaleReturnDetails Where HeaderID=" & lblID.Text)
            cnn.executeSQL("DELETE From BillSundryDetails Where HeaderID=" & lblID.Text & " AND vType='SALERETURN'")
        End If
        ...
        For i = 0 To Grid.Rows.Count - 1
            ...
            '-------------------------------
            strSQL = "INSERT INTO SaleReturnDetails (HeaderID, ProductID, Qty, Nos, Rate, Amount) VALUES("
        ...
        For i = 0 To grdTax.Rows.Count - 1
            ...
            & Val(lblHeaderID.Text) & ",'SALERETURN'," & Val(grdTax.Rows(i).Cells(0).Value) & "," & Val(grdTax.Rows(i).Cells(2).Value) & "," & Val(grdTax.Rows(i).Cells(3).Value) & ")"
            cnn.executeSQL(strSQL)
        Next
    End Sub
    ...
    Private Sub GetDetails()
        ...
        If cmbProduct.Text <> "" Then
            Dim strSQL As String
            strSQL = "SELECT dbo.Products.ProductName, dbo.Products.SalePrice, dbo.ProductStock.Qty FROM dbo.Products INNER JOIN dbo.ProductStock ON dbo.Products.ID = dbo.ProductStock.ProductID" _
    ...
    Private Sub frmSaleReturn_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        strSQL = "SELECT SaleReturnNo from Para Where MasterCompanyID = " & Val(MDI.lblMasterCompanyID.Text) _
        ...
        If dr.Read Then
            If Not IsDBNull(dr.Item("SaleReturnNo")) Then
                txtInvoiceNo.Text = dr.Item("SaleReturnNo")
            Else
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        If lblID.Text <> "" Then
            For i = 0 To Grid.Rows.Count - 1
                ...
                & " WHERE (dbo.SaleReturnDetails.HeaderID = " & Val(lblID.Text) & ") AND (dbo.SaleReturnDetails.ProductID = " & Val(Grid.Rows(i).Cells(0).Value) & ")" _
                ...
                If dr.Read Then
                    If ((Val(dr.Item("StockQty")) + Val(Grid.Rows(i).Cells(3).Value)) - Val(dr.Item("SaleReturnQty"))) < 0 Then
        ...
        If lblID.Text = "" Then
           ...
           & " AND MasterBranchID = " & Val(MDI.lblMasterBranchID.Text)
            ...
            cmd.Connection.Close()
            '---------------------------
            strSQL = "INSERT INTO SaleReturnHeader (RefNo, RefDate, AccountID, TotalQty, TotalNos,TotalAmount, BillAmount, Remarks, MasterCompanyID, MasterBranchID, CreatedBy, CreatedDate) OutPut Inserted.ID as HeaderID VALUES (" _
            ...
            cmd.Connection.Close()
            '==================================
            strSQL = "UPDATE Para SET SaleReturnNo = (SaleReturnNo + 1) Where MasterCompanyID = " & Val(MDI.lblMasterCompanyID.Text) _
            & " AND MasterBranchID = " & Val(MDI.lblMasterBranchID.Text)
            cnn.executeSQL(strSQL)
            '==================================
            Call cnn.GenerateLog("Sale Return", "Add", 0, Val(txtInvoiceNo.Text) & ", Dt. " & txtDate.Text)
        Else
            strSQL = "UPDATE SaleReturnHeader SET RefNo=" & cnn.SQLquote(txtInvoiceNo.Text) & ", RefDate='" & cnn.GetDate(txtDate.Text) & "'" _
            ...
            Call cnn.GenerateLog("Sale Return", "Edit", lblID.Text, Val(txtInvoiceNo.Text) & ", Dt. " & txtDate.Text)
        End If
    ...
    Private Sub cmbSupplier_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbSupplier.SelectedIndexChanged
        If cmbSupplier.Text <> "" Then
            Dim strSQL As String
            strSQL = "Select City from tblAccount Where tType='BUYER' AND ID=" & Val(cmbSupplier.SelectedValue)
            Dim cmd As Data.Odbc.OdbcCommand = New Data.Odbc.OdbcCommand(strSQL, cnn.cnn)
            If cmd.Connection.State = 1 Then cmd.Connection.Close()
            cmd.Connection.Open()
            Dim dr As Data.Odbc.OdbcDataReader = cmd.ExecuteReader
